...
import LogoEvmos from '../../assets/evmos.png';
import { SigningStargateClient } from "@cosmjs/stargate";
import { checkIbcListReturnAlias } from '../../utils'

function BalanceEvmos() {

    const [AllBalancesEvmos, setAllBalancesEvmos] = useState()
  const ibcListsEvmos = [
    {
  ...
  useEffect(() => {
    if (!window.keplr) {
      ...
      async function getAddress() {
        ...
        .then(data => {
        setAllBalancesEvmos(data);
        console.log("data evmos", data)
  });
      }
      getAddress();
    }
      ...
      <div className="bg-grey border border-cyan-300 border-b-4 rounded-lg shadow-xl p-5">
        <div className="flex flex-row items-center">
          ...
          <div className="flex-1 text-center md:text-center align-center">
            ...
            <img src={LogoEvmos} width="70" className="mx-auto"/>
            <ul>
      {AllBalancesEvmos && AllBalancesEvmos.map(wallet => (
        <li className="text-white" >
          - { checkIbcListReturnAlias(wallet.denom, ibcListsEvmos) }: { wallet.amount }
        </li>
      ))}
    </ul>
          </div>
